...
/**
 * Phase 6 SubPhase 1 Step 2 - Level A2 - Task B
 * Fill Frenzy: Fill 6 gaps with report words
 */
...
export default function Phase6SP1Step2RemedialA2TaskB() {
  ...
  const handleSubmit = async () => {
    ...
    SENTENCES.forEach((s, i) => {
      if (answers[i] === s.blank) correct++
    })
    setScore(correct)
    setSubmitted(true)
    sessionStorage.setItem('phase6_sp1_step2_remedial_a2_taskb_score', correct.toString())
    try {
      await phase6API.logRemedialActivity(2, 'A2', 'B', correct, SENTENCES.length, 0, 1)
    } catch (e) {
  ...
  return (
    <Box sx={{ maxWidth: 900, mx: 'auto', p: 3 }}>
      ...
      <Paper elevation={2} sx={{ p: 3, mb: 3, borderRadius: 2 }}>
        <CharacterMessage
          speaker="Ms. Mabrouki"
          message="Fill Frenzy! Choose the correct word from the list to complete each sentence. Use the word bank to help you."
        />
      </Paper>
      ...
      <Stack spacing={2} sx={{ mb: 3 }}>
        {SENTENCES.map((s, idx) => {
          ...
          const isWrong = submitted && answers[idx] !== s.blank
          return (
            <Paper
              key={idx}
              elevation={1}
            ...
            >
              ...
              <Typography variant="body1" sx={{ fontWeight: 500 }}>{s.after}</Typography>
              {isWrong && (
              ...
              {isCorrect && (
                <Typography variant="body2" sx={{ color: '#27ae60', fontWeight: 'bold' }}>✓</Typography>
              )}
            </Paper>
          )
        })}
      ...
      {!submitted ? (
        ...
        <Paper elevation={3} sx={{ p: 3, textAlign: 'center', backgroundColor: '#f0faf4', border: '2px solid #27ae60', borderRadius: 2 }}>
          ...
          <Typography variant="body1" sx={{ mb: 2 }}>
            {score >= 5 ? 'Excellent work!' : score >= 3 ? 'Good effort! Keep practicing.' : 'Review the words and try again.'}
          </Typography>
          <Button
            variant="contained"
            onClick={() => navigate('/phase6/subphase/1/step/2/remedial/a2/task/c')}
            size="large"
            sx={{ backgroundColor: '#27ae60', '&:hover': { backgroundColor: '#1e8449' } }}
          >
            Next: Task C →
