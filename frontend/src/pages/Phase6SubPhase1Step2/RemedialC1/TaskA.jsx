...
/**
 * Phase 6 SubPhase 1 Step 2 - Level C1 - Task A
 * Debate Simulation: Simulate dialogue on report value
 * Word Bank: balanced evaluation, transparency, continuous improvement, evidence-based, actionable
 * Skeptic: "Why weaknesses?" → You: "[Balanced evaluation] shows [transparency]."
 * Skeptic: "Just successes?" → You: "No — [evidence-based] analysis drives [continuous improvement]."
 */
...
export default function Phase6SP1Step2RemedialC1TaskA() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await phase6API.logRemedialActivity(2, 'C1', 'A', correct, Object.keys(CORRECT_ANSWERS).length, 0, 1)
    } catch (e) {
  ...
  return (
    <Box sx={{ maxWidth: 900, mx: 'auto', p: 3 }}>
      ...
      >
        ...
        <Typography variant="body1">Simulate a dialogue defending the value of balanced reporting</Typography>
      </Paper>

      <Paper elevation={2} sx={{ p: 3, mb: 3, borderRadius: 2 }}>
        <CharacterMessage
          speaker="Ms. Mabrouki"
          message="In this debate simulation, a skeptic questions the value of a balanced report. Fill in the gaps with the correct advanced terms to defend professional report writing."
        />
      </Paper>
        ...
        <Stack spacing={2}>
          ...
          <Box sx={{ display: 'flex', gap: 2, alignItems: 'flex-start' }}>
            ...
            <Paper elevation={1} sx={{ p: 2, flex: 1, borderRadius: 2, backgroundColor: '#fdecea' }}>
              ...
              <Typography variant="body1" sx={{ mt: 0.5, fontStyle: 'italic' }}>
                "Why show weaknesses in the report? It makes us look bad."
              </Typography>
            </Paper>
          </Box>
            ...
            <Paper elevation={1} sx={{ p: 2, flex: 1, borderRadius: 2, backgroundColor: '#f0faf4' }}>
              ...
              {submitted && (answers[0] !== 'balanced evaluation' || answers[1] !== 'transparency') && (
                <Typography variant="body2" color="error" sx={{ mt: 0.5 }}>
                  Correct: "Balanced evaluation shows transparency."
                </Typography>
              )}
            </Paper>
          </Box>
          ...
          <Box sx={{ display: 'flex', gap: 2, alignItems: 'flex-start' }}>
            ...
            <Paper elevation={1} sx={{ p: 2, flex: 1, borderRadius: 2, backgroundColor: '#fdecea' }}>
              <Typography variant="caption" color="text.secondary">Skeptic</Typography>
              <Typography variant="body1" sx={{ mt: 0.5, fontStyle: 'italic' }}>
                "Just write the successes. No one wants to read about problems."
              </Typography>
            </Paper>
          </Box>
          ...
          <Box sx={{ display: 'flex', gap: 2, alignItems: 'flex-start', flexDirection: 'row-reverse' }}>
            ...
            <Paper elevation={1} sx={{ p: 2, flex: 1, borderRadius: 2, backgroundColor: '#f0faf4' }}>
              ...
              {submitted && (answers[2] !== 'evidence-based' || answers[3] !== 'continuous improvement') && (
                <Typography variant="body2" color="error" sx={{ mt: 0.5 }}>
                  Correct: "No — evidence-based analysis drives continuous improvement."
                </Typography>
              )}
            </Paper>
          </Box>
      ...
      {!submitted ? (
        ...
        <Paper elevation={3} sx={{ p: 3, textAlign: 'center', backgroundColor: '#f0faf4', border: '2px solid #27ae60', borderRadius: 2 }}>
          ...
          <Typography variant="body1" sx={{ mb: 2 }}>
            {score === 4 ? 'Excellent! Perfect use of advanced report vocabulary!' : 'Good effort! Review the advanced terms and their meanings.'}
          </Typography>
          <Button
            variant="contained"
            onClick={() => navigate('/phase6/subphase/1/step/2/remedial/c1/task/b')}
            size="large"
            sx={{ backgroundColor: '#27ae60', '&:hover': { backgroundColor: '#1e8449' } }}
          >
            Next: Task B →
