...
const GUIDED_QUESTIONS = [
  { id: 1, question: 'Immediate action?', example: 'Secure alternative performer swiftly.' },
  { id: 2, question: 'Communication strategy?', example: 'Issue sincere apology.' },
  { id: 3, question: 'Audience impact?', example: 'Mitigate damage with transparency.' },
  { id: 4, question: 'Backup plan?', example: 'Contingency plans essential.' },
  { id: 5, question: 'Team coordination?', example: 'Inform team promptly.' },
  { id: 6, question: 'Positive spin?', example: 'Turn crisis into opportunity.' },
  { id: 7, question: 'Long-term prevention?', example: 'Strengthen future preparedness.' },
  { id: 8, question: 'Overall outcome?', example: 'Maintain trust through professionalism.' }
]

export default function Phase5Step1RemedialC1TaskB() {
  ...
  const handleSubmit = async () => {
    ...
    answers.forEach(answer => {
      ...
      if (words.length >= 10) correctCount++ // C1 level: sophisticated sentences
    })

    setScore(correctCount)
    setSubmitted(true)
    sessionStorage.setItem('phase5_step1_remedial_c1_taskB_score', correctCount.toString())
    try {
      await phase5API.logRemedialActivity(1, 'C1', 'B', correctCount, 8, 0)
    } catch (error) {
  ...
  const handleContinue = () => {
    navigate('/phase5/subphase/1/step/1/remedial/c1/task/c')
  }
  ...
  return (
    <Box sx={{ maxWidth: 1000, mx: 'auto', p: 3 }}>
      <Paper elevation={0} sx={{ p: 3, mb: 3, background: 'linear-gradient(135deg, #e74c3c 0%, #c0392b 100%)', color: 'white' }}>
        ...
        <Typography variant="body1">Write an 8-sentence analytical paragraph on handling last-minute issues</Typography>
      </Paper>
      <Paper elevation={2} sx={{ p: 3, mb: 3 }}>
        <CharacterMessage speaker="Ms. Mabrouki" message="Welcome to Analysis Odyssey! Write an analytical paragraph with C1-level nuance, cohesive language, and sophisticated vocabulary. Each sentence should be 10+ words." />
      </Paper>
      {!submitted ? (
        <>
