...
/**
 * Phase 5 Step 1 - Remedial B2 - Task A: Role-Play RPG
 * Complete dialogue solving the problem
 */

const WORD_BANK_ORIGINAL = ['alternative', 'because', 'urgent', 'sorry', 'solution', 'fix', 'change']

const SENTENCES = [
  'Ms. Mabrouki: Singer canceled.',
  'You: Find _______ singer _______ _______ .',
  'SKANDER: What message?',
  'You: Send _______ message and _______ .',
  'You: _______ by _______ schedule.'
]

const CORRECT_ANSWERS = {
  'g_1_0': 'alternative', 'g_1_1': 'because', 'g_1_2': 'urgent',
  'g_3_0': 'sorry', 'g_3_1': 'solution',
  'g_4_0': 'fix', 'g_4_1': 'change'
}

export default function Phase5Step1RemedialB2TaskA() {
  ...
  const handleSubmit = async () => {
    ...
    setScore(finalScore)
    setSubmitted(true)
    sessionStorage.setItem('phase5_step1_remedial_b2_taskA_score', finalScore.toString())
    try {
      await phase5API.logRemedialActivity(1, 'B2', 'A', finalScore, 7, 0)
    } catch (error) {
  ...
  const handleContinue = () => {
    navigate('/phase5/subphase/1/step/1/remedial/b2/task/b')
  }
  ...
  return (
    <Box sx={{ maxWidth: 1200, mx: 'auto', p: 3 }}>
      <Paper elevation={0} sx={{ p: 3, mb: 3, background: 'linear-gradient(135deg, #e74c3c 0%, #c0392b 100%)', color: 'white' }}>
        ...
        <Typography variant="h6" gutterBottom>Task A: Role-Play RPG</Typography>
        <Typography variant="body1">Complete dialogue solving the problem. Role-play to level up!</Typography>
      </Paper>
      <Paper elevation={2} sx={{ p: 3, mb: 3 }}>
        <CharacterMessage speaker="Ms. Mabrouki" message="Welcome to Role-Play RPG! Complete the dialogue by filling in the gaps. Use the word bank to find the right words!" />
      </Paper>
      {!submitted && (
        <Box>
