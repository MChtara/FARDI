...
/**
 * Phase 5 SubPhase 2 Step 3 - Level A2 - Task A: Term Treasure Hunt
 * Match 8 instruction terms to simple definitions/pictures
 * Gamified as "Term Treasure Hunt" - Hunt and drag to "treasure chest"
 */
...
const MATCHING_PAIRS = [
  ...
  { word: 'first', definition: 'Step one' },
...
export default function Phase5SubPhase2Step3RemedialA2TaskA() {
  ...
  const handleSubmit = async () => {
    ...
    setScore(finalScore)
    setSubmitted(true)
    sessionStorage.setItem('phase5_subphase2_step3_remedial_a2_taskA_score', finalScore.toString())

    try {
      await phase5API.logRemedialActivity(3, 'A2', 'A', finalScore, 8, 2) // step 3, subphase 2
    } catch (error) {
  ...
  const handleContinue = () => {
    navigate('/phase5/subphase/2/step/3/remedial/a2/task/b')
  }
    ...
    <Box sx={{ maxWidth: 1200, mx: 'auto', p: 3 }}>
      <Paper elevation={0} sx={{ p: 3, mb: 3, background: 'linear-gradient(135deg, #3498db 0%, #2980b9 100%)', color: 'white' }}>
        ...
        <Typography variant="body1">
          Match 8 instruction terms to their definitions. Hunt and drag to treasure chest!
        </Typography>
      </Paper>
      ...
      <Paper elevation={2} sx={{ p: 3, mb: 3 }}>
        <CharacterMessage
          speaker="Ms. Mabrouki"
          message="Welcome to Term Treasure Hunt! Match each instruction term to its definition. Click a word, then click its matching definition. Complete all matches to unlock the treasure!"
        />
      </Paper>
        ...
        <>
          <Paper elevation={3} sx={{ p: 4, mb: 3, backgroundColor: 'success.lighter', textAlign: 'center' }}>
            ...
            <Typography variant="body1" sx={{ mt: 1 }}>
              {score >= 8 ? 'Excellent! All matches correct! Treasure unlocked!' : 'Good work! Let\'s continue to the next task.'}
            </Typography>
          </Paper>
