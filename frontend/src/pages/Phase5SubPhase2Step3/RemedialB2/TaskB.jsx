...
/**
 * Phase 5 SubPhase 2 Step 3 - Remedial B2 - Task B: Explain Expedition
 * Write an 8-sentence explanation of effective volunteer instructions
 */
...
export default function Phase5SubPhase2Step3RemedialB2TaskB() {
  ...
  const handleSubmit = async () => {
    // Evaluate: each sentence should have 8+ words and show B2 grammar
    let correctCount = 0
    ...
    try {
      await phase5API.logRemedialActivity(3, 'B2', 'B', correctCount, 8, 2) // step 3, subphase 2
    } catch (error) {
      ...
      {!submitted ? (
        <>
          ...
          <Paper elevation={2} sx={{ p: 3, mb: 3 }}>
            ...
            <TextField fullWidth multiline rows={4} variant="outlined" placeholder="Write your explanation here..." value={answers[currentIndex]} onChange={(e) => handleAnswerChange(e.target.value)} sx={{ mb: 2 }} />
