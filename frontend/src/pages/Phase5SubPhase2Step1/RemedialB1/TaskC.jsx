...
/**
 * Phase 5 SubPhase 2 Step 1 - Remedial B1 - Task C: Wordshake Quiz
...
export default function Phase5SubPhase2Step1RemedialB1TaskC() {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitted(true)
    sessionStorage.setItem('phase5_subphase2_step1_remedial_b1_taskC_score', correctCount.toString())

    try {
      await phase5API.logRemedialActivity(1, 'B1', 'C', correctCount, 6, 2) // subphase 2
    } catch (error) {
  ...
  const handleContinue = () => {
    // Navigate to score calculation or next step
