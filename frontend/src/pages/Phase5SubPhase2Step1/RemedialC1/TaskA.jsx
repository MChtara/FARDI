...
/**
 * Phase 5 SubPhase 2 Step 1 - Remedial C1 - Task A: Debate Simulation
 * Simulate dialogue discussing best ways to instruct volunteers
 */

const WORD_BANK_ORIGINAL = ['please', 'sequencing', 'polite', 'clear', 'safety', 'empathy']

const SENTENCES = [
  'Opponent: Instructions important?',
  'You: [Clear] and [polite] instructions essential.',
  'Opponent: How structure?',
  'You: Use [sequencing] words and emphasize [safety].'
]
...
export default function Phase5SubPhase2Step1RemedialC1TaskA() {
  ...
  const handleSubmit = async () => {
    ...
    setScore(finalScore)
    setSubmitted(true)
    sessionStorage.setItem('phase5_subphase2_step1_remedial_c1_taskA_score', finalScore.toString())
    try {
      await phase5API.logRemedialActivity(1, 'C1', 'A', finalScore, 4, 2) // subphase 2
    } catch (error) {
  ...
  const handleContinue = () => {
    navigate('/phase5/subphase/2/step/1/remedial/c1/task/b')
  }
  ...
  return (
    <Box sx={{ maxWidth: 1200, mx: 'auto', p: 3 }}>
      <Paper elevation={0} sx={{ p: 3, mb: 3, background: 'linear-gradient(135deg, #3498db 0%, #2980b9 100%)', color: 'white' }}>
        ...
        <Typography variant="body1">Simulate dialogue discussing best ways to instruct volunteers. Show advanced discussion skills!</Typography>
      </Paper>
      <Paper elevation={2} sx={{ p: 3, mb: 3 }}>
        <CharacterMessage speaker="Ms. Mabrouki" message="Welcome to Debate Simulation! Complete the debate dialogue about instructing volunteers. Use advanced vocabulary like 'sequencing', 'empathy', and 'safety'!" />
      </Paper>
      {!submitted && (
        <Box>
