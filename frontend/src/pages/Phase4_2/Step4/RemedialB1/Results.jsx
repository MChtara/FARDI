...
/**
 * Phase 4.2 Step 4 - Level B1 Remedial Results
 * Shows combined results from Tasks A, B, and C
 * Pass threshold: 15/19 points
 */

const PASS_THRESHOLD = 15 // 15/19 to proceed
...
export default function Phase4_2Step4RemedialB1Results() {
  ...
  const handleContinue = () => {
    if (results.passed) {
      ...
      sessionStorage.removeItem('phase4_2_step4_b1_taskB')
      sessionStorage.removeItem('phase4_2_step4_b1_taskC')
      navigate('/phase4_2/step/4/remedial/b1/taskA')
    }
  }

  const progressPercent = (results.total / 19) * 100
    ...
    <Box sx={{ maxWidth: 1000, mx: 'auto', p: 3 }}>
      ...
      >
        <Typography variant="h5" gutterBottom>
          Phase 4.2 Step 4 - Level B1 Remedial
        </Typography>
      ...
      >
        ...
        <Alert severity={results.passed ? 'success' : 'warning'} sx={{ mt: 2 }}>
          {results.passed ? (
            <Typography>
              You scored <strong>{results.total}/19</strong>, which is above the required <strong>{PASS_THRESHOLD}/19</strong> to proceed!
            </Typography>
          ) : (
            <Typography>
              You scored <strong>{results.total}/19</strong>. You need at least <strong>{PASS_THRESHOLD}/19</strong> to proceed. Please try again!
            </Typography>
          )}
        </Alert>
      </Paper>
