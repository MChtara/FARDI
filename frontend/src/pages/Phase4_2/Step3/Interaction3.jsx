...
// Target words for Phase 4.2 Step 3 - Social Media vocabulary
const TARGET_WORDS = [
  'hashtag',
  'caption',
  'viral',
...
export default function Phase4_2Step3Interaction3() {
  ...
  const handleContinue = () => {
    // Calculate total score from all 3 interactions in Step 3
    const cumulativeScore = parseInt(sessionStorage.getItem('phase4_2_step3_score') || '0') // From Int 1 & 2
    const int3Score = parseInt(sessionStorage.getItem('phase4_2_step3_int3_score') || '0')

    const totalScore = cumulativeScore + int3Score
    const totalMax = 10 + TARGET_WORDS.length // Int1&2: 5+5=10, Int3: 10 target words
    const percentage = (totalScore / totalMax) * 100

    // Store total score for Step 3
    ...
    <Box sx={{ maxWidth: 1000, mx: 'auto', p: 3 }}>
      ...
      <Paper elevation={1} sx={{ p: 3, mb: 3, backgroundColor: 'info.lighter' }}>
        ...
        <Stack spacing={1}>
          ...
          <Typography variant="body2">
            • Think of words related to social media: hashtag, caption, viral, engagement, emoji, tag, story, call-to-action, thread, reach
          </Typography>
          <Typography variant="body2">
