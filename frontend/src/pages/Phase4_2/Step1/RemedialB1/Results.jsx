...
/**
 * Phase 4.2 Step 1 - Level B1 Remedial Results
 * Shows combined results from Tasks A through F
 */

const PASS_THRESHOLD = 8 // 8/10 to proceed

export default function Phase4_2RemedialB1Results() {
  ...
  useEffect(() => {
    ...
    const taskAScore = parseFloat(sessionStorage.getItem('phase4_2_remedial_b1_taskA_score') || '0')
    const taskBScore = parseFloat(sessionStorage.getItem('phase4_2_remedial_b1_taskB_score') || '0')
    const taskCScore = parseFloat(sessionStorage.getItem('phase4_2_remedial_b1_taskC_score') || '0')
    const taskDScore = parseFloat(sessionStorage.getItem('phase4_2_remedial_b1_taskD_score') || '0')
    const taskEScore = parseFloat(sessionStorage.getItem('phase4_2_remedial_b1_taskE_score') || '0')
    const taskFScore = parseFloat(sessionStorage.getItem('phase4_2_remedial_b1_taskF_score') || '0')

    const totalScore = taskAScore + taskBScore + taskCScore + taskDScore + taskEScore + taskFScore
  ...
  const handleContinue = () => {
    if (results.passed) {
      ...
      sessionStorage.removeItem('phase4_2_remedial_b1_taskA_score')
      sessionStorage.removeItem('phase4_2_remedial_b1_taskB_score')
      sessionStorage.removeItem('phase4_2_remedial_b1_taskC_score')
      sessionStorage.removeItem('phase4_2_remedial_b1_taskD_score')
      sessionStorage.removeItem('phase4_2_remedial_b1_taskE_score')
      sessionStorage.removeItem('phase4_2_remedial_b1_taskF_score')
      navigate('/dashboard')
    } else {
      // Retry from Task A
      sessionStorage.removeItem('phase4_2_remedial_b1_taskA_score')
      ...
      sessionStorage.removeItem('phase4_2_remedial_b1_taskE_score')
      sessionStorage.removeItem('phase4_2_remedial_b1_taskF_score')
      navigate('/phase4_2/step/1/remedial/b1/taskA')
    }
  }
    ...
    <Box sx={{ maxWidth: 1000, mx: 'auto', p: 3 }}>
      ...
      >
        <Typography variant="h5" gutterBottom>
          Phase 4.2 Step 1 - Level B1 Remedial
        </Typography>
      ...
      <Paper elevation={2} sx={{ p: 3, mb: 3 }}>
        <CharacterMessage
          ...
          message={
            results.passed
              ? "Outstanding! You've demonstrated strong B1-level skills in social media language and grammar. Well done!"
              : "You're making progress, but need more practice. Review the feedback and try again. You've got this!"
          }
        />
      </Paper>
