...
const DIALOGUE_MESSAGES = [
  { id: 1, sender: 'MS. MABROUKI', text: 'What for post?', type: 'question' },
  { id: 2, sender: 'YOU', template: 'Use ______ ______ ______.', blanks: ['hashtag', 'because', 'viral'], type: 'response' },
  { id: 3, sender: 'SKANDER', text: 'And ______?', blanks: ['caption'], type: 'response' },
  { id: 4, sender: 'YOU', template: '______ with ______ and ______ friend.', blanks: ['caption', 'emoji', 'tag'], type: 'response' }
]
...
export default function Phase4_2RemedialA2TaskA() {
  ...
  const handleComplete = () => {
    setCompleted(true)
    sessionStorage.setItem('phase4_2_remedial_a2_taskA_score', score)
    fetch('/api/phase4/remedial/log', {
      method: 'POST', headers: { 'Content-Type': 'application/json' }, credentials: 'include',
      body: JSON.stringify({ phase: '4.2', step: 1, level: 'A2', task: 'A', score, max_score: 6, completed: true })
    }).catch(err => console.error('Log error:', err))
  }

  const renderMessageContent = (message) => {
    if (message.type === 'question') {
      return <Typography variant="body1" sx={{ color: '#2c3e50', fontWeight: 600 }}>{message.text}</Typography>
    }

    if (message.type === 'response') {
      if (message.sent) {
        const parts = message.template ? message.template.split(/_{3,}/) : [message.text]
        return <Typography variant="body1" sx={{ color: '#2c3e50', fontWeight: 600 }}>{parts.map((part, idx) => (
          <React.Fragment key={idx}>
            {part}
            {idx < parts.length - 1 && message.blanks && (
              <strong style={{
    ...
    return (
      <Box sx={{ maxWidth: 1200, mx: 'auto', p: 3 }}>
        <Paper elevation={0} sx={{ p: 3, mb: 3, background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)', color: 'white' }}>
          <Typography variant="h4" gutterBottom fontWeight="bold" sx={{ color: 'white' }}>Phase 4.2 Step 1: Engage - Remedial Practice</Typography>
          <Typography variant="h5" sx={{ color: 'white' }}>Level A2 - Task A: Dialogue Adventure Complete! 🎉</Typography>
        </Paper>
        <Paper elevation={6} sx={{ p: 4, textAlign: 'center', mb: 3 }}>
          <Typography variant="h3" gutterBottom fontWeight="bold" sx={{ color: '#667eea' }}>{score} / 6</Typography>
          <Typography variant="h6" sx={{ color: 'text.primary' }}>Points Earned</Typography>
          <Typography variant="body1" sx={{ mt: 2, color: 'text.primary' }}>Adventure Progress: {((score / 6) * 100).toFixed(0)}% Complete</Typography>
        </Paper>
        <Box sx={{ textAlign: 'center' }}>
          <Button variant="contained" size="large" onClick={() => navigate('/phase4_2/step/1/remedial/a2/taskB')} sx={{ py: 2, px: 6, fontSize: '1.1rem', fontWeight: 'bold', background: 'linear-gradient(135deg, #27ae60 0%, #229954 100%)', '&:hover': { background: 'linear-gradient(135deg, #229954 0%, #1e8449 100%)' } }}>
            Continue to Task B →
          </Button>
        </Box>
      </Box>
  ...
  return (
    <Box sx={{ maxWidth: 1200, mx: 'auto', p: 3 }}>
      <Paper elevation={0} sx={{ p: 3, mb: 3, background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)', color: 'white' }}>
        <Typography variant="h4" gutterBottom fontWeight="bold" sx={{ color: 'white' }}>Phase 4.2 Step 1: Engage - Remedial Practice</Typography>
        <Typography variant="h5" gutterBottom sx={{ color: 'white' }}>Level A2 - Task A: Dialogue Adventure 💬</Typography>
        <Typography variant="body1" sx={{ color: 'white' }}>Complete the dialogue about social media!</Typography>
      </Paper>
      ...
      <Paper elevation={4} sx={{ maxWidth: 600, mx: 'auto', bgcolor: '#E5DDD5', backgroundImage: 'repeating-linear-gradient(45deg, transparent, transparent 10px, rgba(0,0,0,.03) 10px, rgba(0,0,0,.03) 20px)', borderRadius: 2, overflow: 'hidden' }}>
        <Box sx={{ bgcolor: '#667eea', color: 'white', p: 1.5, display: 'flex', alignItems: 'center', gap: 2 }}>
          <Avatar sx={{ bgcolor: '#25D366', width: 36, height: 36 }}><Typography variant="caption" fontWeight="bold" sx={{ color: 'white' }}>A2</Typography></Avatar>
          <Box sx={{ flex: 1 }}>
            <Typography variant="subtitle2" fontWeight="bold" sx={{ color: 'white' }}>Social Media Team Chat</Typography>
            <Typography variant="caption" sx={{ fontSize: '0.7rem', color: 'white' }}>Ms. Mabrouki, Skander, You</Typography>
          </Box>
