 ...
 * Tasks: A-F (6 tasks) = 60 points total, average to /10 scale
...
export default function RemedialB2Results() {
  ...
  const calculateFinalScore = async () => {
    ...
    console.log('\n' + '='.repeat(60))
    console.log('PHASE 4.2 STEP 1 - REMEDIAL B2 - FINAL RESULTS')
    ...
    console.log('Task E (Conditionals):', taskEScore, '/10')
    console.log('Task F (Passives):', taskFScore, '/10')
    console.log('-'.repeat(60))
    ...
    if (passed) {
      ...
      console.log('❌ FAILED - Student will repeat Phase 4.2 Step 1 Remedial Level B2')
    }
    console.log('='.repeat(60) + '\n')
    ...
    try {
      ...
      if (data.success) {
        console.log('Phase 4.2 Step 1 B2 Final score logged to backend:', data.data)
      }
    } catch (error) {
  ...
  const handleRedirect = () => {
    // Clear B2 scores
    sessionStorage.removeItem('remedial_phase4_2_step1_b2_taskA_score')
    sessionStorage.removeItem('remedial_phase4_2_step1_b2_taskB_score')
    sessionStorage.removeItem('remedial_phase4_2_step1_b2_taskC_score')
    sessionStorage.removeItem('remedial_phase4_2_step1_b2_taskD_score')
    sessionStorage.removeItem('remedial_phase4_2_step1_b2_taskE_score')
    sessionStorage.removeItem('remedial_phase4_2_step1_b2_taskF_score')

    if (scores.passed) {
      navigate('/dashboard')
    } else {
      navigate('/phase4_2/step1/remedial/b2/taskA')
    }
  }
    ...
    <Box sx={{ maxWidth: 1200, mx: 'auto', p: 3 }}>
        ...
        {scores.passed ? (
          <Alert severity="success" sx={{ mt: 2, backgroundColor: 'rgba(46, 204, 113, 0.9)' }}>
            ...
            <Typography variant="body1" sx={{ mt: 1 }}>
              Excellent work! You've mastered the B2 level social media skills with strong understanding and communication abilities.
            </Typography>
          </Alert>
        ) : (
          <Alert severity="warning" sx={{ mt: 2, backgroundColor: 'rgba(230, 126, 34, 0.9)' }}>
            ...
            <Typography variant="body1" sx={{ mt: 1 }}>
              Don't worry! You'll restart from Task A to improve your B2-level social media vocabulary and grammar.
            </Typography>
          </Alert>
        )}
      </Paper>
      ...
      <Paper elevation={3} sx={{ p: 3, mb: 3 }}>
        ...
        <Stack spacing={2} sx={{ mt: 2 }}>
          ...
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', p: 2, backgroundColor: '#ecf0f1', borderRadius: 1 }}>
            <Typography variant="body1" sx={{ color: '#2c3e50' }}>Task B: Writing</Typography>
          ...
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', p: 2, backgroundColor: '#ecf0f1', borderRadius: 1 }}>
            <Typography variant="body1" sx={{ color: '#2c3e50' }}>Task E: Conditionals</Typography>
            <Typography variant="h6" fontWeight="bold" sx={{ color: scores.taskE >= 8 ? '#27ae60' : '#e67e22' }}>
              {scores.taskE} / 10
            </Typography>
          </Box>

          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', p: 2, backgroundColor: '#ecf0f1', borderRadius: 1 }}>
            <Typography variant="body1" sx={{ color: '#2c3e50' }}>Task F: Passives</Typography>
            <Typography variant="h6" fontWeight="bold" sx={{ color: scores.taskF >= 8 ? '#27ae60' : '#e67e22' }}>
              {scores.taskF} / 10
            </Typography>
          </Box>
