...
/**
 * Phase 5 SubPhase 2 Step 4 - Level A2 - Task B: Fill Quest
 * Fill 8 gaps with instruction terms
 */
...
export default function Phase5SubPhase2Step4RemedialA2TaskB() {
  ...
  const handleSubmit = async () => {
    ...
    setScore(finalScore)
    setSubmitted(true)
    sessionStorage.setItem('phase5_subphase2_step4_remedial_a2_taskB_score', finalScore.toString())

    try {
      await phase5API.logRemedialActivity(4, 'A2', 'B', finalScore, 8, 2) // step 4, subphase 2
    } catch (error) {
  ...
  const handleContinue = () => {
    navigate('/phase5/subphase/2/step/4/remedial/a2/task/c')
  }
  ...
  return (
    <Box sx={{ maxWidth: 1200, mx: 'auto', p: 3 }}>
      <Paper elevation={0} sx={{ p: 3, mb: 3, background: 'linear-gradient(135deg, #3498db 0%, #2980b9 100%)', color: 'white' }}>
        ...
        <Typography variant="body1">Fill in 8 gaps with instruction terms. Fill to quest through levels!</Typography>
      </Paper>

      <Paper elevation={2} sx={{ p: 3, mb: 3 }}>
        <CharacterMessage speaker="Ms. Mabrouki" message="Welcome to Fill Quest! Fill in the gaps with words from the word bank. Complete all sentences to advance through the quest!" />
      </Paper>
