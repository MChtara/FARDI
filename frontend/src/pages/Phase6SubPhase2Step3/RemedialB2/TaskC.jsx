...
export default function Phase6SP2Step3RemB2TaskC() {
  ...
  const handleDefClick = (pair) => {
    ...
    setWrongPair(null)
    if (selectedTerm) {
      attemptMatch(selectedTerm, pair)
    }
  }
  ...
  return (
    <Box sx={{ maxWidth: 900, mx: 'auto', p: 3 }}>
      <Paper elevation={0} sx={{ p: 3, mb: 3, background: 'linear-gradient(135deg, #8e44ad 0%, #6c3483 100%)', color: 'white', borderRadius: 2 }}>
        <Typography variant="h5" gutterBottom fontWeight="bold">Step 3: Remedial B2 - Task C</Typography>
        <Typography variant="body1">Matching Game: Feedback Terms to Definitions</Typography>
      </Paper>

      <Alert severity="info" sx={{ mb: 3 }}>
        Click a term on the left, then click its matching definition on the right to create a pair.
      </Alert>
      ...
      <Box sx={{ display: 'flex', gap: 2, mb: 3 }}>
        ...
        {/* Center connector column — hidden on mobile */}
