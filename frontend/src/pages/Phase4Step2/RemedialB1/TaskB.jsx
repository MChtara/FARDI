...
export default function RemedialB1TaskB() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/phase4/step2/remedial/b1/evaluate-definitions', {
        method: 'POST',
      ...
      setResults(result)
      sessionStorage.setItem('remedial_step2_b1_taskB_score', result.total_score)
  ...
  return (
    <Box sx={{ maxWidth: 1200, mx: 'auto', p: 3 }}>
      <Paper elevation={0} sx={{ p: 3, mb: 3, backgroundColor: 'primary.light', color: 'primary.contrastText' }}>
        <Typography variant="h4" gutterBottom>Phase 4 - Step 2: Remedial Activities</Typography>
        ...
        <Box sx={{ mt: 4 }}>
          <Paper elevation={3} sx={{ p: 3, mb: 3 }}>
            ...
            <Box sx={{ mt: 3, textAlign: 'center' }}>
              <Button variant="contained" color="primary" onClick={() => navigate('/app/phase4/step2/remedial/b1/taskC')} size="large">
                Continue to Task C →
              </Button>
            </Box>
          </Paper>
