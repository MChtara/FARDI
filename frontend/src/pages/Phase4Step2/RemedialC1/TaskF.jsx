...
/**
 * Phase 4 Step 2 - Remedial C1 - Task F: Clause Conquest
...
const CLAUSE_SENTENCES = [
  {
    ...
    sentenceParts: [
      ...
      { type: 'blank', id: 'clause', placeholder: 'which is designed to sell' },
  ...
  },
  {
    ...
    sentenceParts: [
      ...
      { type: 'blank', id: 'clause', placeholder: 'that are employed effectively' },
  ...
  },
  {
    ...
    sentenceParts: [
      ...
      { type: 'blank', id: 'clause', placeholder: 'which focus on groups' },
  ...
  },
  {
    ...
    sentenceParts: [
      ...
      { type: 'blank', id: 'clause', placeholder: 'that are created freshly' },
  ...
  },
  {
    ...
    sentenceParts: [
      ...
      { type: 'blank', id: 'clause', placeholder: 'which have been developed' },
  ...
  },
  {
    ...
    sentenceParts: [
      ...
      { type: 'blank', id: 'clause', placeholder: 'by which stories are told' },
...
export default function RemedialC1TaskF() {
  ...
  const handleSubmitAll = async () => {
    ...
    // Evaluate with LLM
    try {
      const response = await fetch('/api/phase4/step2/remedial/c1/evaluate-clauses-batch', {
        method: 'POST',
      ...
      if (data.success && data.results) {
        const evaluatedResults = sentences.map((sent, index) => ({
          ...sent,
        ...
        sessionStorage.setItem('remedial_step2_c1_taskF_score', totalScore)
      ...
      // Fallback: Local evaluation - just check if blanks are filled
      const evaluatedResults = sentences.map(sent => {
        const hasBlanks = CLAUSE_SENTENCES.find(c => c.id === sent.sentenceId).sentenceParts.filter(p => p.type === 'blank').length
        const filledBlanks = Object.keys(answers).filter(k => k.startsWith(`${sent.sentenceId}_`) && answers[k].trim().length > 0).length

        return {
          ...sent,
          score: filledBlanks >= hasBlanks ? 1 : 0,
          feedback: filledBlanks >= hasBlanks ? 'All blanks filled!' : 'Please fill in all blanks for a complete sentence.'
        }
      })
