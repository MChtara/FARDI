...
/**
 * Phase 5 SubPhase 2 Step 5 - Remedial B1 - Task A: Negotiation Gap Fill
 * Fill gaps correcting grammar/structure in volunteer instruction dialogue
 */
...
export default function Phase5SubPhase2Step5RemedialB1TaskA() {
  ...
  const handleSubmit = async () => {
    ...
    setScore(finalScore)
    setSubmitted(true)
    sessionStorage.setItem('phase5_subphase2_step5_remedial_b1_taskA_score', finalScore.toString())

    try {
      await phase5API.logRemedialActivity(5, 'B1', 'A', finalScore, 4, 2) // step 5, subphase 2
    } catch (error) {
  ...
  const handleContinue = () => {
    navigate('/phase5/subphase/2/step/5/remedial/b1/task/b')
  }
  ...
  return (
    <Box sx={{ maxWidth: 1200, mx: 'auto', p: 3 }}>
      <Paper elevation={0} sx={{ p: 3, mb: 3, background: 'linear-gradient(135deg, #3498db 0%, #2980b9 100%)', color: 'white' }}>
        ...
        <Typography variant="body1">Fill gaps correcting grammar/structure in volunteer instruction dialogue</Typography>
      </Paper>

      <Paper elevation={2} sx={{ p: 3, mb: 3 }}>
        <CharacterMessage speaker="Ms. Mabrouki" message="Welcome to Negotiation Battle! Complete the dialogue correcting grammar and structure by filling in the gaps. Use the word bank!" />
      </Paper>
