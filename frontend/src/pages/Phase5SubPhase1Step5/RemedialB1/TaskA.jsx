...
const WORD_BANK_ORIGINAL = ['have', 'problem', 'backup', 'lights', 'because', 'emergency']

const SENTENCES = [
  'SKANDER: Problem?',
  'You: Lights [have] [problem].',
  'Emna: Solution?',
  'You: Use [backup] [lights] [because] emergency.'
]
...
export default function Phase5Step5RemedialB1TaskA() {
  ...
  const handleSubmit = async () => {
    ...
    setScore(finalScore)
    setSubmitted(true)
    sessionStorage.setItem('phase5_step5_remedial_b1_taskA_score', finalScore.toString())
    try {
      await phase5API.logRemedialActivity(5, 'B1', 'A', finalScore, 5, 0)
    } catch (error) {
  ...
  const handleContinue = () => {
    navigate('/phase5/subphase/1/step/5/remedial/b1/task/b')
  }
  ...
  return (
    <Box sx={{ maxWidth: 1200, mx: 'auto', p: 3 }}>
      <Paper elevation={0} sx={{ p: 3, mb: 3, background: 'linear-gradient(135deg, #e74c3c 0%, #c0392b 100%)', color: 'white' }}>
        ...
        <Typography variant="body1">Fill gaps correcting grammar/structure in crisis dialogue</Typography>
      </Paper>
      <Paper elevation={2} sx={{ p: 3, mb: 3 }}>
        <CharacterMessage speaker="Ms. Mabrouki" message="Welcome! Complete the crisis dialogue by filling in the gaps with correct grammar and structure!" />
      </Paper>
      {!submitted && (
        <Box>
