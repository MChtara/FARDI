...
const TARGET_VOCABULARY = ['spelling', 'correct', 'feedback', 'constructive', 'specific']
...
export default function Phase6SP2Step5Int2() {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true)
    try {
      const result = await phase6API.evaluateSpellingSubPhase2(text.trim())
      ...
      setSubmitted(true)
      sessionStorage.setItem('phase6_sp2_step5_interaction2_score', evalResult.score.toString())
      sessionStorage.setItem('phase6_sp2_step5_interaction2_level', evalResult.level)
    } catch (error) {
      console.error('Evaluation error, using fallback:', error)
      const fb = fallbackEvaluate(text)
      setEvaluation(fb)
      setSubmitted(true)
      sessionStorage.setItem('phase6_sp2_step5_interaction2_score', fb.score.toString())
      sessionStorage.setItem('phase6_sp2_step5_interaction2_level', fb.level)
  ...
  return (
    <Box sx={{ maxWidth: 1200, mx: 'auto', p: 3 }}>
      <Paper elevation={0} sx={{ p: 3, mb: 3, background: 'linear-gradient(135deg, #8e44ad 0%, #6c3483 100%)', color: 'white', borderRadius: 2 }}>
        ...
        <Typography variant="body1">Tone and Politeness Fix</Typography>
      </Paper>

      <Paper elevation={2} sx={{ p: 3, mb: 3, borderRadius: 2 }}>
        <Typography variant="body1" sx={{ fontStyle: 'italic', mb: 2 }}>Lilia: "Good spelling fixes! Now fix the tone and politeness issues in this feedback. Make it constructive and professional."</Typography>
      </Paper>

      <Alert severity="info" sx={{ mb: 3 }}>
        <Typography variant="body2"><strong>Hint:</strong> Look for rude or blunt language and replace it with polite, constructive phrases. Use words like 'suggest', 'consider', 'you might', 'it would help if'.</Typography>
      </Alert>
      ...
      {evaluation && (
        <Paper elevation={3} sx={{ p: 3, mb: 3, backgroundColor: 'success.lighter', border: '2px solid', borderColor: 'success.main', borderRadius: 2 }}>
          ...
          {submitted && (
            <Button variant="contained" onClick={() => navigate('/phase6/subphase/2/step/5/interaction/3')} fullWidth size="large"
