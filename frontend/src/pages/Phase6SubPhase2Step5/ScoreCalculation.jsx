...
export default function Phase6SP2Step5Score() {
  ...
  useEffect(() => {
    const calc = async () => {
      const i1 = parseInt(sessionStorage.getItem('phase6_sp2_step5_interaction1_score') || '0')
      const i2 = parseInt(sessionStorage.getItem('phase6_sp2_step5_interaction2_score') || '1')
      const i3 = parseInt(sessionStorage.getItem('phase6_sp2_step5_interaction3_score') || '0')
      ...
      let proceed = i2 >= 3
      try {
        const result = await phase6API.calculateStepScore(5, { interaction1_score: i1, interaction2_score: i2, interaction3_score: i3 }, 2)
      ...
      sessionStorage.setItem('phase6_sp2_step5_total_score', total.toString())
      sessionStorage.setItem('phase6_sp2_step5_remedial_level', remedialLevel)
      setLoading(false)
    }
    calc()
  }, [])
  ...
  const handleContinue = () => {
    if (shouldProceed) {
      navigate('/phase6/complete')
    } else {
      navigate(`/phase6/subphase/2/step/5/remedial/${level.toLowerCase()}/task/a`)
    }
  }
    ...
    <Box sx={{ maxWidth: 1000, mx: 'auto', p: 3 }}>
      ...
      <Paper elevation={2} sx={{ p: 3, mb: 3, borderRadius: 2 }}>
        ...
        <Stack spacing={2} sx={{ mt: 2 }}>
          {[{ label: 'Interaction 1', score: scores.i1, max: 1 }, { label: 'Interaction 2 (Spelling Correction)', score: scores.i2, max: 4 }, { label: 'Interaction 3', score: scores.i3, max: 1 }].map((item, idx) => (
