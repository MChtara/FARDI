...
const PAIRS = [
  { word: 'feedback', definition: 'Speech bubble — what people say' },
  { word: 'positive', definition: 'Happy face — good things' },
  { word: 'suggestion', definition: 'Light bulb — idea to make better' },
  { word: 'strength', definition: 'Muscle — what is good' },
  { word: 'weakness', definition: 'Broken chain — needs improvement' },
  { word: 'improve', definition: 'Arrow up — make better' }
]

function shuffle(arr) { const a = [...arr]; for (let i = a.length - 1; i > 0; i--) { const j = Math.floor(Math.random() * (i + 1)); [a[i], a[j]] = [a[j], a[i]] } return a }

export default function Phase6SP2Step2RemA2TaskA() {
  ...
  const handleSubmit = async () => {
    ...
    setScore(correct)
    setSubmitted(true)
    sessionStorage.setItem('phase6_sp2_step2_remedial_a2_taska_score', correct.toString())
    try { await phase6API.logRemedialActivity(2, 'A2', 'A', correct, PAIRS.length, 0, 2) } catch (e) { console.error(e) }
  }

  return (
    <Box sx={{ maxWidth: 1100, mx: 'auto', p: 3 }}>
      <Paper elevation={0} sx={{ p: 3, mb: 3, background: 'linear-gradient(135deg, #8e44ad 0%, #6c3483 100%)', color: 'white', borderRadius: 2 }}>
        <Typography variant="h5" gutterBottom fontWeight="bold">Step 2: Remedial A2 - Task A</Typography>
        <Typography variant="body1">Match Race: Feedback Words and Pictures</Typography>
      </Paper>
      {!submitted ? (
        <>
          ...
          <Grid container spacing={3}>
            ...
            <Grid item xs={12} md={5}>
              <Paper elevation={1} sx={{ p: 2 }}>
                <Typography variant="h6" gutterBottom textAlign="center">Pictures/Definitions</Typography>
                <Stack spacing={1.5}>
        ...
        <Paper elevation={3} sx={{ p: 4, textAlign: 'center', backgroundColor: '#e8f8f0', borderRadius: 2 }}>
          ...
          <Button variant="contained" onClick={() => navigate('/phase6/subphase/2/step/2/remedial/a2/task/b')} size="large"
