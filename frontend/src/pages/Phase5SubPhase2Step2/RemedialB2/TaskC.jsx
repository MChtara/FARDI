...
/**
 * Phase 5 SubPhase 2 Step 2 - Remedial B2 - Task C: Matching Game
...
export default function Phase5SubPhase2Step2RemedialB2TaskC() {
  ...
  const handleSubmit = async () => {
    ...
    setScore(finalScore)
    setSubmitted(true)
    sessionStorage.setItem('phase5_subphase2_step2_remedial_b2_taskC_score', finalScore.toString())

    try {
      await phase5API.logRemedialActivity(2, 'B2', 'C', finalScore, 8, 2) // step 2, subphase 2
    } catch (error) {
  ...
  const handleContinue = () => {
    navigate('/phase5/subphase/2/step/2/remedial/b2/task/d')
  }
