...
/**
 * Phase 6 SubPhase 1 Step 3 - Level A2 - Task B
 * Fill Quest: Fill in 8 gaps with report terms
 */
...
export default function Phase6SP1Step3RemedialA2TaskB() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await phase6API.logRemedialActivity(3, 'A2', 'B', correct, SENTENCES.length, 0, 1)
    } catch (e) {
  ...
  return (
    <Box sx={{ maxWidth: 900, mx: 'auto', p: 3 }}>
      <Paper elevation={0} sx={{ p: 3, mb: 3, background: 'linear-gradient(135deg, #27ae60 0%, #1e8449 100%)', color: 'white', borderRadius: 2 }}>
        ...
        <Typography variant="h6">Task B: Fill Quest</Typography>
        <Typography variant="body1">Fill in 8 gaps with report terms</Typography>
      </Paper>
      ...
      {!submitted ? (
        ...
        <Paper elevation={3} sx={{ p: 3, textAlign: 'center', backgroundColor: '#f0faf4', border: '2px solid #27ae60', borderRadius: 2 }}>
          ...
          <Button variant="contained" onClick={() => navigate('/phase6/subphase/1/step/3/remedial/a2/task/c')} size="large"
