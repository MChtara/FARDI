...
/**
 * Phase 6 SubPhase 1 Step 3 - Level B2 - Task C
 * Matching Game: Match 8 terms to advanced definitions
 */

const PAIRS = [
  { term: 'success', definition: 'Positive outcome' },
  { term: 'challenge', definition: 'Obstacle faced' },
  { term: 'feedback', definition: 'Participant opinions' },
  { term: 'improve', definition: 'Enhance future events' },
  { term: 'strength', definition: 'Area of excellence' },
  { term: 'weakness', definition: 'Area needing work' },
  { term: 'recommend', definition: 'Suggest action' },
  { term: 'summary', definition: 'Concise overview' }
]
...
export default function Phase6SP1Step3RemedialB2TaskC() {
  ...
  const handleTermClick = (idx) => {
    ...
    if (matchedTerms.has(idx)) {
      const m = { ...matches }; delete m[idx]; setMatches(m); return
    }
    setSelectedTerm(idx)
  }
  ...
  const handleSubmit = async () => {
    ...
    setScore(correct)
    setSubmitted(true)
    sessionStorage.setItem('phase6_sp1_step3_remedial_b2_taskc_score', correct.toString())
    try {
      await phase6API.logRemedialActivity(3, 'B2', 'C', correct, PAIRS.length, 0, 1)
    } catch (e) {
  ...
  return (
    <Box sx={{ maxWidth: 1100, mx: 'auto', p: 3 }}>
      <Paper elevation={0} sx={{ p: 3, mb: 3, background: 'linear-gradient(135deg, #27ae60 0%, #1e8449 100%)', color: 'white', borderRadius: 2 }}>
        ...
        <Typography variant="body1">Match 8 report terms to their advanced definitions</Typography>
      </Paper>
      ...
      {!submitted ? (
        <>
          <Alert severity="info" sx={{ mb: 3 }}>Click a term (left) to select it, then click its matching definition (right).</Alert>
        ...
        <>
          <Paper elevation={3} sx={{ p: 3, mb: 3, backgroundColor: '#f0faf4', border: '2px solid #27ae60', borderRadius: 2 }}>
            ...
            <Stack spacing={1}>
              {PAIRS.map((p, idx) => {
                ...
                return (
                  <Box key={idx} sx={{ p: 1, borderRadius: 1, backgroundColor: isCorrect ? '#c8e6c9' : '#ffcdd2' }}>
          ...
          <Stack direction="row" justifyContent="flex-end">
            <Button variant="contained" onClick={() => navigate('/phase6/subphase/1/step/3/remedial/b2/task/d')} size="large"
