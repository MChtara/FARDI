...
const WORD_BANK_ORIGINAL = ['emergency', 'contingency', 'backup', 'announce', 'update', 'transparent']

const SENTENCES = [
  'Opponent: Response?',
  'You: [Emergency] requires [contingency].',
  'Opponent: Communication?',
  'You: [Transparent] [announce] and [update].'
]
...
export default function Phase5Step4RemedialC1TaskA() {
  ...
  const handleSubmit = async () => {
    ...
    setScore(finalScore)
    setSubmitted(true)
    sessionStorage.setItem('phase5_step4_remedial_c1_taskA_score', finalScore.toString())
    try {
      await phase5API.logRemedialActivity(4, 'C1', 'A', finalScore, 5, 0)
    } catch (error) {
  ...
  const handleContinue = () => {
    navigate('/phase5/subphase/1/step/4/remedial/c1/task/b')
  }
  ...
  return (
    <Box sx={{ maxWidth: 1200, mx: 'auto', p: 3 }}>
      <Paper elevation={0} sx={{ p: 3, mb: 3, background: 'linear-gradient(135deg, #e74c3c 0%, #c0392b 100%)', color: 'white' }}>
        ...
        <Typography variant="body1">Simulate dialogue on crisis response</Typography>
      </Paper>
      <Paper elevation={2} sx={{ p: 3, mb: 3 }}>
        <CharacterMessage speaker="Ms. Mabrouki" message="Welcome to Debate Simulation! Complete the debate dialogue using advanced vocabulary and argumentation!" />
      </Paper>
      {!submitted && (
        <Box>
