...
export default function Phase5Step4RemedialB2TaskB() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await phase5API.logRemedialActivity(4, 'B2', 'B', correctCount, 8, 0)
    } catch (error) {
  ...
  const handleContinue = () => {
    navigate('/phase5/subphase/1/step/4/remedial/b2/task/c')
  }
  ...
  return (
    <Box sx={{ maxWidth: 1000, mx: 'auto', p: 3 }}>
      <Paper elevation={0} sx={{ p: 3, mb: 3, background: 'linear-gradient(135deg, #e74c3c 0%, #c0392b 100%)', color: 'white' }}>
        ...
        <Typography variant="h6" gutterBottom>Task B: Crisis Communication Plan</Typography>
