...
export default function Phase5Step4RemedialB1TaskC() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await phase5API.logRemedialActivity(4, 'B1', 'C', correctCount, 6, 0)
    } catch (error) {
  ...
  const handleContinue = async () => {
    const taskAScore = parseInt(sessionStorage.getItem('phase5_step4_remedial_b1_taskA_score') || '0')
    const taskBScore = parseInt(sessionStorage.getItem('phase5_step4_remedial_b1_taskB_score') || '0')
    const taskCScore = parseInt(sessionStorage.getItem('phase5_step4_remedial_b1_taskC_score') || '0')
    const totalScore = taskAScore + taskBScore + taskCScore
    const maxScore = 3 + 8 + 6
    const threshold = Math.ceil(maxScore * 0.8)
    const passed = totalScore >= threshold

    try {
      await phase5API.calculateRemedialScore(4, 'B1', {
    ...
    if (passed) {
      ...
      navigate('/phase5/subphase/1/step/4/remedial/b1/task/a')
    }
  }
