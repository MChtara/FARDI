      ...
      >
        ...
        >
          ...
          <Box sx={{ mt: 4, display: 'flex', gap: 2, justifyContent: 'center' }}>
            <Button
              onClick={() => {
                // Calculate total remedial C1 score
                const taskScores = {
                ...
                const maxScore = 48 // 6 points per task × 8 tasks
                const percentage = (totalScore / maxScore) * 100

                console.log('=== REMEDIAL C1 COMPLETION ===')
                console.log('Task Scores:', taskScores)
                console.log(`Total: ${totalScore}/${maxScore} (${percentage.toFixed(1)}%)`)

                // Navigate based on performance
                if (percentage >= 80) {
