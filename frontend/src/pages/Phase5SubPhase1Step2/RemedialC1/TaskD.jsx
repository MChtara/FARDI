...
export default function Phase5Step2RemedialC1TaskD() {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitted(true)
    sessionStorage.setItem('phase5_step2_remedial_c1_taskD_score', correctCount.toString())
    try {
      await phase5API.logRemedialActivity(2, 'C1', 'D', correctCount, 6, 0)
    } catch (error) {
  ...
  const handleContinue = async () => {
    ...
    const maxScore = 3 + 8 + 6 + 6
    const threshold = Math.ceil(maxScore * 0.8)
    const passed = totalScore >= threshold

    try {
      await phase5API.calculateRemedialScore(2, 'C1', {
    ...
    if (passed) {
      ...
      navigate('/phase5/subphase/1/step/2/remedial/c1/task/a')
    }
  }
  ...
  return (
    <Box sx={{ maxWidth: 1000, mx: 'auto', p: 3 }}>
      <Paper elevation={0} sx={{ p: 3, mb: 3, background: 'linear-gradient(135deg, #3498db 0%, #2980b9 100%)', color: 'white' }}>
        ...
        <Typography variant="body1">Critique 6 crisis approaches with nuanced analysis</Typography>
      </Paper>
