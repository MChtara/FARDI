...
const SENTENCES = [
  ...
  'You: Immediate ___ deployment.',
  'Opponent: Communication?',
  'You: Transparent ___ and ___.'
]
...
export default function Phase5Step2RemedialC1TaskA() {
  ...
  const handleSubmit = async () => {
    ...
    setScore(finalScore)
    setSubmitted(true)
    sessionStorage.setItem('phase5_step2_remedial_c1_taskA_score', finalScore.toString())
    try {
      await phase5API.logRemedialActivity(2, 'C1', 'A', finalScore, 3, 0)
    } catch (error) {
  ...
  const handleContinue = () => {
    navigate('/phase5/subphase/1/step/2/remedial/c1/task/b')
  }
  ...
  return (
    <Box sx={{ maxWidth: 1200, mx: 'auto', p: 3 }}>
      <Paper elevation={0} sx={{ p: 3, mb: 3, background: 'linear-gradient(135deg, #3498db 0%, #2980b9 100%)', color: 'white' }}>
        ...
        <Typography variant="body1">Simulate dialogue on crisis strategies</Typography>
      </Paper>
      <Paper elevation={2} sx={{ p: 3, mb: 3 }}>
        <CharacterMessage speaker="Ms. Mabrouki" message="Welcome to Debate Simulation! Complete the debate dialogue using advanced vocabulary!" />
      </Paper>
      {!submitted && (
        <Box>
