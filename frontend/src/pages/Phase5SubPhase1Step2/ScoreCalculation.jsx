...
/**
 * Phase 5 Step 2: Score Calculation & Remedial Routing
 */

export default function Phase5Step2ScoreCalculation() {
  const navigate = useNavigate()
  const [loading, setLoading] = useState(true)
  const [calculating, setCalculating] = useState(false)
  const [scores, setScores] = useState({
    interaction1Game: 0,
    interaction1Writing: 0,
    interaction2: 0,
    interaction3Game: 0,
  ...
  const calculateScore = async () => {
    ...
    try {
      const interaction1GameScore = parseInt(sessionStorage.getItem('phase5_step2_interaction1_game_score') || '0')
      const interaction1WritingScore = parseInt(sessionStorage.getItem('phase5_step2_interaction1_writing_score') || '0')
      const interaction2Score = parseInt(sessionStorage.getItem('phase5_step2_interaction2_score') || '0')
      const interaction3GameScore = parseInt(sessionStorage.getItem('phase5_step2_interaction3_game_score') || '0')
      const interaction3RevisionScore = parseInt(sessionStorage.getItem('phase5_step2_interaction3_revision_score') || '0')

      const totalScore = interaction1GameScore + interaction1WritingScore + interaction2Score + interaction3GameScore + interaction3RevisionScore

      setScores({
        interaction1Game: interaction1GameScore,
        interaction1Writing: interaction1WritingScore,
        interaction2: interaction2Score,
        interaction3Game: interaction3GameScore,
        interaction3Revision: interaction3RevisionScore,
        total: totalScore
      })

      const result = await phase5API.calculateStepScore(2, {
        interaction1_score: interaction1GameScore,
        interaction1_writing_score: interaction1WritingScore,
      ...
      if (result.success && result.data) {
        ...
        sessionStorage.setItem('phase5_step2_total_score', totalScore.toString())
        sessionStorage.setItem('phase5_step2_remedial_level', remedialLevel)
      } else {
      ...
      const interaction2Score = parseInt(sessionStorage.getItem('phase5_step2_interaction2_score') || '2')
      const remedialLevel = determineRemedialLevel(interaction2Score)
      setRouting({
        remedialLevel,
        shouldProceed: interaction2Score >= 3,
  ...
  const handleContinue = () => {
    ...
    if (routing.shouldProceed) {
      navigate('/phase5/subphase/1/step/3')
    } else {
      const levelLower = routing.remedialLevel.toLowerCase()
      navigate(`/phase5/subphase/1/step/2/remedial/${levelLower}/task/a`)
    }
  }
    ...
    <Box sx={{ maxWidth: 1000, mx: 'auto', p: 3 }}>
      ...
      <Paper elevation={2} sx={{ p: 3, mb: 3 }}>
        ...
        <Stack spacing={2} sx={{ mt: 2 }}>
          ...
          <Card variant="outlined">
            <CardContent>
              <Typography variant="body1" gutterBottom><strong>Interaction 1 (Writing):</strong> {scores.interaction1Writing} / 5 points</Typography>
              <LinearProgress variant="determinate" value={(scores.interaction1Writing / 5) * 100} sx={{ mt: 1, height: 8, borderRadius: 1 }} color="primary" />
            </CardContent>
          </Card>
          <Card variant="outlined">
            <CardContent>
              <Typography variant="body1" gutterBottom><strong>Interaction 2 (Explanation):</strong> {scores.interaction2} / 5 points</Typography>
          ...
          <Card variant="outlined">
            <CardContent>
              <Typography variant="body1" gutterBottom><strong>Interaction 3 (Revision):</strong> {scores.interaction3Revision} / 5 points</Typography>
              <LinearProgress variant="determinate" value={(scores.interaction3Revision / 5) * 100} sx={{ mt: 1, height: 8, borderRadius: 1 }} color="primary" />
            </CardContent>
          </Card>
