...
export default function Phase5Step2RemedialA2Results() {
  ...
  const calculateFinalScore = async () => {
    ...
    try {
      await phase5API.logRemedialActivity(2, 'A2', 'final', total, {
  ...
  const handleRedirect = () => {
    if (scores.passed) {
      ...
      navigate('/phase5/step2/remedial-a2/task-a')
    }
  }
    ...
    <Box sx={{ maxWidth: 1000, mx: 'auto', p: 3 }}>
      ...
      <Paper elevation={2} sx={{ p: 3, mb: 3 }}>
        ...
        <Stack spacing={1} sx={{ mt: 2 }}>
          <Box><Typography>Task A (Match Race): {scores.taskA} / 6</Typography><LinearProgress variant="determinate" value={(scores.taskA/6)*100} /></Box>
          <Box><Typography>Task B (Fill Frenzy): {scores.taskB} / 6</Typography><LinearProgress variant="determinate" value={(scores.taskB/6)*100} /></Box>
          <Box><Typography>Task C (Sentence Builder): {scores.taskC} / 6</Typography><LinearProgress variant="determinate" value={(scores.taskC/6)*100} /></Box>
        </Stack>
      </Paper>
      ...
      <Alert severity={scores.passed ? 'success' : 'warning'} sx={{ mb: 3 }}>
        {scores.passed 
          ...
          : `Redirecting to repeat A2 remedial in ${countdown} seconds...`}
      </Alert>

      <Button variant="contained" color={scores.passed ? 'success' : 'primary'} onClick={handleRedirect} size="large" fullWidth>
        {scores.passed ? 'Continue to Dashboard' : 'Retry A2 Remedial'}
      </Button>
    </Box>
  )
}
