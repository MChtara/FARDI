...
export default function Phase5Step2RemedialB1TaskA() {
  ...
  const handleSubmit = async () => {
    ...
    setScore(finalScore)
    setSubmitted(true)
    sessionStorage.setItem('phase5_step2_remedial_b1_taskA_score', finalScore.toString())
    try {
      await phase5API.logRemedialActivity(2, 'B1', 'A', finalScore, 3, 0)
    } catch (error) {
  ...
  const handleContinue = () => {
    navigate('/phase5/subphase/1/step/2/remedial/b1/task/b')
  }
