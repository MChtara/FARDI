...
const GUIDED_QUESTIONS = [
  ...
  { id: 2, question: 'Solution?', example: 'Use backup.' },
  { id: 3, question: 'Announce?', example: 'Tell audience.' },
  { id: 4, question: 'Why backup?', example: 'Because fast.' },
  { id: 5, question: 'Update?', example: 'Post on social.' },
  { id: 6, question: 'Thank people?', example: 'Thank for patience.' }
]
...
export default function Phase5Step2RemedialB1TaskB() {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitted(true)
    sessionStorage.setItem('phase5_step2_remedial_b1_taskB_score', correctCount.toString())
    try {
      await phase5API.logRemedialActivity(2, 'B1', 'B', correctCount, 6, 0)
    } catch (error) {
  ...
  const handleContinue = () => {
    navigate('/phase5/subphase/1/step/2/remedial/b1/task/c')
  }
  ...
  return (
    <Box sx={{ maxWidth: 1000, mx: 'auto', p: 3 }}>
      <Paper elevation={0} sx={{ p: 3, mb: 3, background: 'linear-gradient(135deg, #3498db 0%, #2980b9 100%)', color: 'white' }}>
        ...
        <Typography variant="body1">Write 6 sentences proposing crisis solutions</Typography>
      </Paper>
      <Paper elevation={2} sx={{ p: 3, mb: 3 }}>
        <CharacterMessage speaker="Ms. Mabrouki" message="Welcome to Proposal Builder! Write sentences proposing solutions for the crisis. Each sentence should have at least 5 words!" />
      </Paper>
      {!submitted ? (
        <>
