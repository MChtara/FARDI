...
/**
 * Phase 5 Step 2 - Remedial B2 - Results Page
...
export default function Phase5Step2RemedialB2Results() {
  ...
  const calculateFinalScore = async () => {
    ...
    try {
      await phase5API.logRemedialActivity(2, 'B2', 'final', total, {
  ...
  const handleRedirect = () => {
    if (scores.passed) {
      ...
      navigate('/phase5/step2/remedial-b2/task-a')
    }
  }
    ...
    <Box sx={{ maxWidth: 1000, mx: 'auto', p: 3 }}>
      ...
      <Paper elevation={2} sx={{ p: 3, mb: 3 }}>
        ...
        <Stack spacing={1} sx={{ mt: 2 }}>
          <Box><Typography>Task A (Role-Play Saga): {scores.taskA} / 5</Typography><LinearProgress variant="determinate" value={(scores.taskA/5)*100} /></Box>
          <Box><Typography>Task B (Instruction Odyssey): {scores.taskB} / 8</Typography><LinearProgress variant="determinate" value={(scores.taskB/8)*100} /></Box>
          <Box><Typography>Task C (Function Match): {scores.taskC} / 8</Typography><LinearProgress variant="determinate" value={(scores.taskC/8)*100} /></Box>
          <Box><Typography>Task D (Spell Quest): {scores.taskD} / 6</Typography><LinearProgress variant="determinate" value={(scores.taskD/6)*100} /></Box>
        </Stack>
      </Paper>
      ...
      <Alert severity={scores.passed ? 'success' : 'warning'} sx={{ mb: 3 }}>
        {scores.passed 
          ...
          : `Redirecting to repeat B2 remedial in ${countdown} seconds...`}
      </Alert>
