...
export default function Phase5Step3RemedialB2TaskD() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await phase5API.logRemedialActivity(3, 'B2', 'D', correctCount, 6, 0)
    } catch (error) {
  ...
  const handleContinue = async () => {
    ...
    const maxScore = 6 + 8 + 8 + 6
    const threshold = Math.ceil(maxScore * 0.8)
    const passed = totalScore >= threshold

    try {
      await phase5API.calculateRemedialScore(3, 'B2', {
    ...
    if (passed) {
      ...
      navigate('/phase5/subphase/1/step/3/remedial/b2/task/a')
    }
  }
