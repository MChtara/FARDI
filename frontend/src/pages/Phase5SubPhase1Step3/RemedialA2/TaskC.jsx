...
export default function Phase5Step3RemedialA2TaskC() {
  ...
  const checkAnswer = (userAnswer, prompt) => {
    ...
    return normalized.includes(termNormalized) && normalized.split(' ').length >= 3
  }
  ...
  const handleSubmit = async () => {
    ...
    try {
      await phase5API.logRemedialActivity(3, 'A2', 'C', correctCount, 6, 0)
    } catch (error) {
  ...
  const handleContinue = async () => {
    ...
    try {
      await phase5API.calculateRemedialScore(3, 'A2', {
    ...
    if (passed) {
      ...
      navigate('/phase5/subphase/1/step/3/remedial/a2/task/a')
    }
  }
  ...
  return (
    <Box sx={{ maxWidth: 1000, mx: 'auto', p: 3 }}>
      <Paper elevation={0} sx={{ p: 3, mb: 3, background: 'linear-gradient(135deg, #9b59b6 0%, #8e44ad 100%)', color: 'white' }}>
        ...
        <Typography variant="body1">Write 6 simple sentences using crisis terms</Typography>
      </Paper>
