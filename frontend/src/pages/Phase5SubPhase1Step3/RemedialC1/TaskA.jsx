...
export default function Phase5Step3RemedialC1TaskA() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await phase5API.logRemedialActivity(3, 'C1', 'A', finalScore, 6, 0)
    } catch (error) {
  ...
  const handleContinue = () => {
    navigate('/phase5/subphase/1/step/3/remedial/c1/task/b')
  }
  ...
  return (
    <Box sx={{ maxWidth: 1200, mx: 'auto', p: 3 }}>
      <Paper elevation={0} sx={{ p: 3, mb: 3, background: 'linear-gradient(135deg, #9b59b6 0%, #8e44ad 100%)', color: 'white' }}>
        ...
        <Typography variant="body1">Simulate dialogue on crisis communication strategies</Typography>
      </Paper>
