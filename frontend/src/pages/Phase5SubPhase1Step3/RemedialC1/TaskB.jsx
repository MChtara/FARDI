...
const GUIDED_QUESTIONS = [
  { id: 1, question: 'Emergency assessment.', example: 'Rapid assessment of crisis scope.' },
  { id: 2, question: 'Contingency deployment.', example: 'Immediate activation of backup systems.' },
  { id: 3, question: 'Transparent communication.', example: 'Honest and timely disclosure.' },
  { id: 4, question: 'Stakeholder management.', example: 'Coordinated updates to all parties.' },
  { id: 5, question: 'Reassurance strategy.', example: 'Calm, solution-focused messaging.' },
  { id: 6, question: 'Resolution monitoring.', example: 'Real-time progress tracking.' },
  { id: 7, question: 'Post-crisis analysis.', example: 'Root-cause evaluation.' },
  { id: 8, question: 'Organizational learning.', example: 'Improved future resilience.' }
]

export default function Phase5Step3RemedialC1TaskB() {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitted(true)
    sessionStorage.setItem('phase5_step3_remedial_c1_taskB_score', correctCount.toString())
    try {
      await phase5API.logRemedialActivity(3, 'C1', 'B', correctCount, 8, 0)
    } catch (error) {
  ...
  const handleContinue = () => {
    navigate('/phase5/subphase/1/step/3/remedial/c1/task/c')
  }
  ...
  return (
    <Box sx={{ maxWidth: 1000, mx: 'auto', p: 3 }}>
      <Paper elevation={0} sx={{ p: 3, mb: 3, background: 'linear-gradient(135deg, #9b59b6 0%, #8e44ad 100%)', color: 'white' }}>
        ...
        <Typography variant="body1">Write an 8-sentence analytical paragraph on crisis communication</Typography>
      </Paper>
      <Paper elevation={2} sx={{ p: 3, mb: 3 }}>
        <CharacterMessage speaker="Ms. Mabrouki" message="Welcome! Write a sophisticated analytical paragraph with C1-level nuance. Each sentence should be 10+ words. Analysis Odyssey through crisis communication!" />
      </Paper>
      {!submitted ? (
        <>
